...
def init_db():
    ...
    if c.fetchone()[0] == 0:
        default_services = [
            ("換屏幕", "手機", 480, 1150, "更換破碎或失效的螢幕，多種屏幕等級可選（零件價格 + $300服務費）"),
            ("換電池", "手機", 380, 550, "更換老化電池，恢復續航力（零件價格 + $300服務費）"),
            ("水浸維修", "手機", 500, 850, "清洗主板、乾燥處理，視損壞程度報價"),
            ("換充電口", "手機", 360, 450, "更換損壞的充電接口（零件價格 + $300服務費）"),
            ("換鏡頭", "手機", 420, 650, "更換前置/後置鏡頭模組（零件價格 + $300服務費）"),
            ("換聽筒/喇叭", "手機", 350, 430, "更換聽筒或揚聲器（零件價格 + $300服務費）"),
            ("換顯示屏", "平板", 580, 1050, "iPad/Android 平板螢幕更換（零件價格 + $300服務費）"),
            ("Switch 維修", "遊戲機", 150, 680, "Joy-Con 飄移、螢幕維修、按鍵更換等"),
            ("換玻璃背蓋", "手機", 380, 520, "更換破碎的玻璃背蓋（零件價格 + $300服務費）"),
            ("軟體維修/解鎖", "手機", 250, 450, "系統修復、忘記密碼解鎖等（純服務費）"),
        ]
    ...
    # 插入預設維修零件選項
    # 定價公式：客戶價格 = 零件成本 + HK$300 維修服務費
    # 零件來源：Refox / Rewa 華強北批發價（港幣換算）
    c.execute("SELECT COUNT(*) FROM repair_part_options")
    if c.fetchone()[0] == 0:
        default_part_options = [
            # service_id, name, price, description, sort_order
            # 換屏幕 (service_id=1)
            # Incell LCD 成本約 HK$180 → 售價 480
            (1, "Incell LCD 屏", 480, "Refox Incell LCD，色彩表現良好，適合日常使用（零件約$180 + 服務費$300）", 1),
            # Hard OLED 成本約 HK$320 → 售價 620
            (1, "Hard OLED 屏", 620, "Rewa Hard OLED，色彩鮮豔對比度高，性價比之選（零件約$320 + 服務費$300）", 2),
            # Soft OLED 成本約 HK$520 → 售價 820
            (1, "Soft OLED 屏", 820, "Rewa Soft OLED，原廠級顯示效果，色彩精準細膩（零件約$520 + 服務費$300）", 3),
            # 原裝拆機屏 成本約 HK$850 → 售價 1150
            (1, "原裝拆機屏", 1150, "Apple 原裝拆機屏幕總成，100%原廠品質（零件約$850 + 服務費$300）", 4),
            # 換電池 (service_id=2)
            # 副廠電池 成本約 HK$80 → 售價 380
            (2, "副廠電池", 380, "Refox 高容量副廠電池，續航表現穩定（零件約$80 + 服務費$300）", 1),
            # 原廠電池 成本約 HK$250 → 售價 550
            (2, "原廠電池", 550, "Rewa Apple 原廠認證電池，健康度100%，續航最佳（零件約$250 + 服務費$300）", 2),
            # 水浸維修 (service_id=3) — 服務費為主，無固定零件成本
            (3, "基礎清洗維修", 500, "主板清洗烘乾 + 基礎檢測修復（含服務費）", 1),
            (3, "深度晶片維修", 850, "超聲波清洗 + 晶片級維修，Refox 專業設備處理（含服務費）", 2),
            # 換充電口 (service_id=4)
            # 副廠充電口 成本約 HK$60 → 售價 360
            (4, "副廠充電接口", 360, "Refox 副廠充電尾插排線，功能穩定（零件約$60 + 服務費$300）", 1),
            # 原廠充電口 成本約 HK$150 → 售價 450
            (4, "原廠充電接口", 450, "Rewa Apple 原廠充電尾插模組，穩定耐用（零件約$150 + 服務費$300）", 2),
            # 換鏡頭 (service_id=5)
            # 副廠鏡頭 成本約 HK$120 → 售價 420
            (5, "副廠鏡頭模組", 420, "Refox 副廠鏡頭模組，成像清晰（零件約$120 + 服務費$300）", 1),
            # 原廠鏡頭 成本約 HK$350 → 售價 650
            (5, "原廠鏡頭模組", 650, "Rewa Apple 原廠鏡頭模組，成像品質最佳（零件約$350 + 服務費$300）", 2),
            # 換聽筒/喇叭 (service_id=6)
            # 副廠 成本約 HK$50 → 售價 350
            (6, "副廠聽筒/喇叭", 350, "Refox 副廠聽筒喇叭模組，功能正常（零件約$50 + 服務費$300）", 1),
            # 原廠 成本約 HK$130 → 售價 430
            (6, "原廠聽筒/喇叭", 430, "Rewa Apple 原廠聽筒喇叭模組，音質最佳（零件約$130 + 服務費$300）", 2),
            # 換顯示屏 (service_id=7) - 平板
            # 副廠 LCD 成本約 HK$280 → 售價 580
            (7, "副廠 LCD 顯示屏", 580, "Refox 平板 LCD 顯示總成，經濟實惠（零件約$280 + 服務費$300）", 1),
            # 原廠屏 成本約 HK$750 → 售價 1050
            (7, "原廠顯示屏", 1050, "Rewa 原廠級別平板顯示總成，色彩精準（零件約$750 + 服務費$300）", 2),
            # Switch 維修 (service_id=8)
            (8, "Joy-Con 搖桿維修", 150, "Joy-Con 飄移修復、按鍵更換（含服務費）", 1),
            (8, "Switch 屏幕更換", 680, "更換 Switch LCD 屏幕總成，Refox 零件（零件約$380 + 服務費$300）", 2),
            (8, "主板晶片維修", 650, "晶片級維修，不開機、充電問題等（含服務費）", 3),
            # 換玻璃背蓋 (service_id=9)
            # 副廠背蓋 成本約 HK$80 → 售價 380
            (9, "副廠玻璃背蓋", 380, "Refox 副廠玻璃背蓋，外觀一致（零件約$80 + 服務費$300）", 1),
            # 原廠背蓋 成本約 HK$220 → 售價 520
            (9, "原廠玻璃背蓋", 520, "Rewa Apple 原廠玻璃背蓋，質感最佳（零件約$220 + 服務費$300）", 2),
            # 軟體維修/解鎖 (service_id=10) — 純服務
            (10, "標準系統維修", 250, "系統修復、軟體問題處理（純服務費）", 1),
            (10, "深度解鎖服務", 450, "忘記密碼、iCloud 解鎖等進階服務（純服務費）", 2),
        ]
    ...
    if c.fetchone()[0] == 0:
        default_prices = [
            # ===== iPhone 16 系列 =====
            # iPhone 16
            ("Apple", "iPhone 16", "128GB", "excellent", 3700, "manual"),
            ("Apple", "iPhone 16", "128GB", "good", 3000, "manual"),
            ("Apple", "iPhone 16", "128GB", "fair", 2200, "manual"),
            ("Apple", "iPhone 16", "256GB", "excellent", 4200, "manual"),
            ("Apple", "iPhone 16", "256GB", "good", 3400, "manual"),
            ("Apple", "iPhone 16", "256GB", "fair", 2500, "manual"),
            # iPhone 16 Plus
            ("Apple", "iPhone 16 Plus", "128GB", "excellent", 4400, "manual"),
            ("Apple", "iPhone 16 Plus", "128GB", "good", 3600, "manual"),
            ("Apple", "iPhone 16 Plus", "128GB", "fair", 2600, "manual"),
            ("Apple", "iPhone 16 Plus", "256GB", "excellent", 4900, "manual"),
            ("Apple", "iPhone 16 Plus", "256GB", "good", 4000, "manual"),
            ("Apple", "iPhone 16 Plus", "256GB", "fair", 2900, "manual"),
            # iPhone 16 Pro
            ("Apple", "iPhone 16 Pro", "128GB", "excellent", 5100, "manual"),
            ("Apple", "iPhone 16 Pro", "128GB", "good", 4100, "manual"),
            ("Apple", "iPhone 16 Pro", "128GB", "fair", 3000, "manual"),
            ("Apple", "iPhone 16 Pro", "256GB", "excellent", 5800, "manual"),
            ("Apple", "iPhone 16 Pro", "256GB", "good", 4700, "manual"),
            ("Apple", "iPhone 16 Pro", "256GB", "fair", 3400, "manual"),
            # iPhone 16 Pro Max
            ("Apple", "iPhone 16 Pro Max", "256GB", "excellent", 6700, "manual"),
            ("Apple", "iPhone 16 Pro Max", "256GB", "good", 5500, "manual"),
            ("Apple", "iPhone 16 Pro Max", "256GB", "fair", 4000, "manual"),
            ("Apple", "iPhone 16 Pro Max", "512GB", "excellent", 7400, "manual"),
            ("Apple", "iPhone 16 Pro Max", "512GB", "good", 6200, "manual"),
            ("Apple", "iPhone 16 Pro Max", "512GB", "fair", 4500, "manual"),

            # ===== iPhone 15 系列 =====
            # iPhone 15
            ("Apple", "iPhone 15", "128GB", "excellent", 2900, "manual"),
            ("Apple", "iPhone 15", "128GB", "good", 2400, "manual"),
            ("Apple", "iPhone 15", "128GB", "fair", 1700, "manual"),
            ("Apple", "iPhone 15", "256GB", "excellent", 3400, "manual"),
            ("Apple", "iPhone 15", "256GB", "good", 2800, "manual"),
            ("Apple", "iPhone 15", "256GB", "fair", 2000, "manual"),
            # iPhone 15 Plus
            ("Apple", "iPhone 15 Plus", "128GB", "excellent", 3500, "manual"),
            ("Apple", "iPhone 15 Plus", "128GB", "good", 2800, "manual"),
            ("Apple", "iPhone 15 Plus", "128GB", "fair", 2000, "manual"),
            ("Apple", "iPhone 15 Plus", "256GB", "excellent", 4000, "manual"),
            ("Apple", "iPhone 15 Plus", "256GB", "good", 3200, "manual"),
            ("Apple", "iPhone 15 Plus", "256GB", "fair", 2300, "manual"),
            # iPhone 15 Pro
            ("Apple", "iPhone 15 Pro", "128GB", "excellent", 4400, "manual"),
            ("Apple", "iPhone 15 Pro", "128GB", "good", 3600, "manual"),
            ("Apple", "iPhone 15 Pro", "128GB", "fair", 2600, "manual"),
            ("Apple", "iPhone 15 Pro", "256GB", "excellent", 5100, "manual"),
            ("Apple", "iPhone 15 Pro", "256GB", "good", 4100, "manual"),
            ("Apple", "iPhone 15 Pro", "256GB", "fair", 3000, "manual"),
            # iPhone 15 Pro Max
            ("Apple", "iPhone 15 Pro Max", "256GB", "excellent", 5700, "manual"),
            ("Apple", "iPhone 15 Pro Max", "256GB", "good", 4700, "manual"),
            ("Apple", "iPhone 15 Pro Max", "256GB", "fair", 3400, "manual"),
            ("Apple", "iPhone 15 Pro Max", "512GB", "excellent", 6300, "manual"),
            ("Apple", "iPhone 15 Pro Max", "512GB", "good", 5200, "manual"),
            ("Apple", "iPhone 15 Pro Max", "512GB", "fair", 3800, "manual"),

            # ===== iPhone 14 系列 =====
            # iPhone 14
            ("Apple", "iPhone 14", "128GB", "excellent", 2000, "manual"),
            ("Apple", "iPhone 14", "128GB", "good", 1600, "manual"),
            ("Apple", "iPhone 14", "128GB", "fair", 1100, "manual"),
            ("Apple", "iPhone 14", "256GB", "excellent", 2400, "manual"),
            ("Apple", "iPhone 14", "256GB", "good", 1900, "manual"),
            ("Apple", "iPhone 14", "256GB", "fair", 1400, "manual"),
            # iPhone 14 Plus
            ("Apple", "iPhone 14 Plus", "128GB", "excellent", 2400, "manual"),
            ("Apple", "iPhone 14 Plus", "128GB", "good", 1900, "manual"),
            ("Apple", "iPhone 14 Plus", "128GB", "fair", 1300, "manual"),
            ("Apple", "iPhone 14 Plus", "256GB", "excellent", 2800, "manual"),
            ("Apple", "iPhone 14 Plus", "256GB", "good", 2200, "manual"),
            ("Apple", "iPhone 14 Plus", "256GB", "fair", 1600, "manual"),
            # iPhone 14 Pro
            ("Apple", "iPhone 14 Pro", "128GB", "excellent", 2900, "manual"),
            ("Apple", "iPhone 14 Pro", "128GB", "good", 2400, "manual"),
            ("Apple", "iPhone 14 Pro", "128GB", "fair", 1700, "manual"),
            ("Apple", "iPhone 14 Pro", "256GB", "excellent", 3400, "manual"),
            ("Apple", "iPhone 14 Pro", "256GB", "good", 2800, "manual"),
            ("Apple", "iPhone 14 Pro", "256GB", "fair", 2000, "manual"),
            # iPhone 14 Pro Max
            ("Apple", "iPhone 14 Pro Max", "128GB", "excellent", 3500, "manual"),
            ("Apple", "iPhone 14 Pro Max", "128GB", "good", 2800, "manual"),
            ("Apple", "iPhone 14 Pro Max", "128GB", "fair", 2000, "manual"),
            ("Apple", "iPhone 14 Pro Max", "256GB", "excellent", 4000, "manual"),
            ("Apple", "iPhone 14 Pro Max", "256GB", "good", 3200, "manual"),
            ("Apple", "iPhone 14 Pro Max", "256GB", "fair", 2300, "manual"),

            # ===== iPhone 13 系列 =====
            # iPhone 13
            ("Apple", "iPhone 13", "128GB", "excellent", 1500, "manual"),
            ("Apple", "iPhone 13", "128GB", "good", 1200, "manual"),
            ("Apple", "iPhone 13", "128GB", "fair", 900, "manual"),
            ("Apple", "iPhone 13", "256GB", "excellent", 1900, "manual"),
            ("Apple", "iPhone 13", "256GB", "good", 1500, "manual"),
            ("Apple", "iPhone 13", "256GB", "fair", 1100, "manual"),
            # iPhone 13 Pro
            ("Apple", "iPhone 13 Pro", "128GB", "excellent", 2000, "manual"),
            ("Apple", "iPhone 13 Pro", "128GB", "good", 1600, "manual"),
            ("Apple", "iPhone 13 Pro", "128GB", "fair", 1100, "manual"),
            ("Apple", "iPhone 13 Pro", "256GB", "excellent", 2400, "manual"),
            ("Apple", "iPhone 13 Pro", "256GB", "good", 1900, "manual"),
            ("Apple", "iPhone 13 Pro", "256GB", "fair", 1400, "manual"),
            # iPhone 13 Pro Max
            ("Apple", "iPhone 13 Pro Max", "128GB", "excellent", 2500, "manual"),
            ("Apple", "iPhone 13 Pro Max", "128GB", "good", 2000, "manual"),
            ("Apple", "iPhone 13 Pro Max", "128GB", "fair", 1400, "manual"),
            ("Apple", "iPhone 13 Pro Max", "256GB", "excellent", 2900, "manual"),
            ("Apple", "iPhone 13 Pro Max", "256GB", "good", 2300, "manual"),
            ("Apple", "iPhone 13 Pro Max", "256GB", "fair", 1700, "manual"),

            # ===== iPhone 12 系列 =====
            # iPhone 12
            ("Apple", "iPhone 12", "64GB", "excellent", 1000, "manual"),
            ("Apple", "iPhone 12", "64GB", "good", 800, "manual"),
            ("Apple", "iPhone 12", "64GB", "fair", 550, "manual"),
            ("Apple", "iPhone 12", "128GB", "excellent", 1200, "manual"),
            ("Apple", "iPhone 12", "128GB", "good", 950, "manual"),
            ("Apple", "iPhone 12", "128GB", "fair", 700, "manual"),
            ("Apple", "iPhone 12", "256GB", "excellent", 1400, "manual"),
            ("Apple", "iPhone 12", "256GB", "good", 1100, "manual"),
            ("Apple", "iPhone 12", "256GB", "fair", 800, "manual"),
            # iPhone 12 Pro
            ("Apple", "iPhone 12 Pro", "128GB", "excellent", 1500, "manual"),
            ("Apple", "iPhone 12 Pro", "128GB", "good", 1200, "manual"),
            ("Apple", "iPhone 12 Pro", "128GB", "fair", 850, "manual"),
            ("Apple", "iPhone 12 Pro", "256GB", "excellent", 1800, "manual"),
            ("Apple", "iPhone 12 Pro", "256GB", "good", 1400, "manual"),
            ("Apple", "iPhone 12 Pro", "256GB", "fair", 1000, "manual"),
            # iPhone 12 Pro Max
            ("Apple", "iPhone 12 Pro Max", "128GB", "excellent", 1900, "manual"),
            ("Apple", "iPhone 12 Pro Max", "128GB", "good", 1500, "manual"),
            ("Apple", "iPhone 12 Pro Max", "128GB", "fair", 1000, "manual"),
            ("Apple", "iPhone 12 Pro Max", "256GB", "excellent", 2200, "manual"),
            ("Apple", "iPhone 12 Pro Max", "256GB", "good", 1700, "manual"),
            ("Apple", "iPhone 12 Pro Max", "256GB", "fair", 1200, "manual"),

            # ===== iPhone 11 系列 =====
            # iPhone 11
            ("Apple", "iPhone 11", "64GB", "excellent", 700, "manual"),
            ("Apple", "iPhone 11", "64GB", "good", 550, "manual"),
            ("Apple", "iPhone 11", "64GB", "fair", 350, "manual"),
            ("Apple", "iPhone 11", "128GB", "excellent", 900, "manual"),
            ("Apple", "iPhone 11", "128GB", "good", 700, "manual"),
            ("Apple", "iPhone 11", "128GB", "fair", 500, "manual"),
            # iPhone 11 Pro
            ("Apple", "iPhone 11 Pro", "64GB", "excellent", 900, "manual"),
            ("Apple", "iPhone 11 Pro", "64GB", "good", 700, "manual"),
            ("Apple", "iPhone 11 Pro", "64GB", "fair", 450, "manual"),
            ("Apple", "iPhone 11 Pro", "256GB", "excellent", 1200, "manual"),
            ("Apple", "iPhone 11 Pro", "256GB", "good", 900, "manual"),
            ("Apple", "iPhone 11 Pro", "256GB", "fair", 600, "manual"),
            # iPhone 11 Pro Max
            ("Apple", "iPhone 11 Pro Max", "64GB", "excellent", 1100, "manual"),
            ("Apple", "iPhone 11 Pro Max", "64GB", "good", 800, "manual"),
            ("Apple", "iPhone 11 Pro Max", "64GB", "fair", 550, "manual"),
            ("Apple", "iPhone 11 Pro Max", "256GB", "excellent", 1400, "manual"),
            ("Apple", "iPhone 11 Pro Max", "256GB", "good", 1000, "manual"),
            ("Apple", "iPhone 11 Pro Max", "256GB", "fair", 700, "manual"),

            # ===== iPhone SE =====
            ("Apple", "iPhone SE (2022)", "64GB", "excellent", 800, "manual"),
            ("Apple", "iPhone SE (2022)", "64GB", "good", 600, "manual"),
            ("Apple", "iPhone SE (2022)", "64GB", "fair", 400, "manual"),
            ("Apple", "iPhone SE (2022)", "128GB", "excellent", 1000, "manual"),
            ("Apple", "iPhone SE (2022)", "128GB", "good", 750, "manual"),
            ("Apple", "iPhone SE (2022)", "128GB", "fair", 500, "manual"),
            ("Apple", "iPhone SE (2020)", "64GB", "excellent", 550, "manual"),
            ("Apple", "iPhone SE (2020)", "64GB", "good", 400, "manual"),
            ("Apple", "iPhone SE (2020)", "64GB", "fair", 250, "manual"),

            # ===== Samsung =====
            ("Samsung", "Galaxy S25 Ultra", "256GB", "excellent", 4800, "manual"),
            ("Samsung", "Galaxy S25 Ultra", "256GB", "good", 3900, "manual"),
            ("Samsung", "Galaxy S25 Ultra", "256GB", "fair", 2800, "manual"),
            ("Samsung", "Galaxy S24 Ultra", "256GB", "excellent", 4200, "manual"),
            ("Samsung", "Galaxy S24 Ultra", "256GB", "good", 3400, "manual"),
            ("Samsung", "Galaxy S24 Ultra", "256GB", "fair", 2400, "manual"),
            ("Samsung", "Galaxy S23 Ultra", "256GB", "excellent", 3200, "manual"),
            ("Samsung", "Galaxy S23 Ultra", "256GB", "good", 2600, "manual"),
            ("Samsung", "Galaxy S23 Ultra", "256GB", "fair", 1800, "manual"),
            ("Samsung", "Galaxy S23", "128GB", "excellent", 2200, "manual"),
            ("Samsung", "Galaxy S23", "128GB", "good", 1700, "manual"),
            ("Samsung", "Galaxy S23", "128GB", "fair", 1200, "manual"),
            ("Samsung", "Galaxy S22 Ultra", "128GB", "excellent", 2000, "manual"),
            ("Samsung", "Galaxy S22 Ultra", "128GB", "good", 1500, "manual"),
            ("Samsung", "Galaxy S22 Ultra", "128GB", "fair", 1000, "manual"),
            ("Samsung", "Galaxy Z Fold6", "512GB", "excellent", 4200, "manual"),
            ("Samsung", "Galaxy Z Fold6", "512GB", "good", 3300, "manual"),
            ("Samsung", "Galaxy Z Fold6", "512GB", "fair", 2300, "manual"),

            # ===== 華為 =====
            ("Huawei", "Mate X5", "512GB", "excellent", 6000, "manual"),
            ("Huawei", "Mate X5", "512GB", "good", 4800, "manual"),
            ("Huawei", "Mate X5", "512GB", "fair", 3200, "manual"),
            ("Huawei", "Mate 60 Pro+", "512GB", "excellent", 4700, "manual"),
            ("Huawei", "Mate 60 Pro+", "512GB", "good", 3700, "manual"),
            ("Huawei", "Mate 60 Pro+", "512GB", "fair", 2500, "manual"),
            ("Huawei", "Mate 60 Pro", "256GB", "excellent", 3500, "manual"),
            ("Huawei", "Mate 60 Pro", "256GB", "good", 2700, "manual"),
            ("Huawei", "Mate 60 Pro", "256GB", "fair", 1800, "manual"),

            # ===== 小米 =====
            ("Xiaomi", "15 Ultra", "256GB", "excellent", 3500, "manual"),
            ("Xiaomi", "15 Ultra", "256GB", "good", 2700, "manual"),
            ("Xiaomi", "15 Ultra", "256GB", "fair", 1800, "manual"),
            ("Xiaomi", "14 Ultra", "256GB", "excellent", 2900, "manual"),
            ("Xiaomi", "14 Ultra", "256GB", "good", 2300, "manual"),
            ("Xiaomi", "14 Ultra", "256GB", "fair", 1500, "manual"),
            ("Xiaomi", "13T Pro", "256GB", "excellent", 1600, "manual"),
            ("Xiaomi", "13T Pro", "256GB", "good", 1300, "manual"),
            ("Xiaomi", "13T Pro", "256GB", "fair", 800, "manual"),

            # ===== iPad 系列 =====
            # iPad Pro M4 (2024)
            ("iPad", "iPad Pro 13 (M4, 2024)", "256GB", "excellent", 5300, "manual"),
            ("iPad", "iPad Pro 13 (M4, 2024)", "256GB", "good", 4300, "manual"),
            ("iPad", "iPad Pro 13 (M4, 2024)", "256GB", "fair", 3000, "manual"),
            ("iPad", "iPad Pro 13 (M4, 2024)", "512GB", "excellent", 5900, "manual"),
            ("iPad", "iPad Pro 13 (M4, 2024)", "512GB", "good", 4800, "manual"),
            ("iPad", "iPad Pro 13 (M4, 2024)", "512GB", "fair", 3400, "manual"),
            ("iPad", "iPad Pro 11 (M4, 2024)", "256GB", "excellent", 4200, "manual"),
            ("iPad", "iPad Pro 11 (M4, 2024)", "256GB", "good", 3400, "manual"),
            ("iPad", "iPad Pro 11 (M4, 2024)", "256GB", "fair", 2400, "manual"),
            ("iPad", "iPad Pro 11 (M4, 2024)", "512GB", "excellent", 4800, "manual"),
            ("iPad", "iPad Pro 11 (M4, 2024)", "512GB", "good", 3900, "manual"),
            ("iPad", "iPad Pro 11 (M4, 2024)", "512GB", "fair", 2800, "manual"),
            # iPad Pro M2 (2022)
            ("iPad", "iPad Pro 12.9 (M2, 2022)", "128GB", "excellent", 3500, "manual"),
            ("iPad", "iPad Pro 12.9 (M2, 2022)", "128GB", "good", 2800, "manual"),
            ("iPad", "iPad Pro 12.9 (M2, 2022)", "128GB", "fair", 1900, "manual"),
            ("iPad", "iPad Pro 12.9 (M2, 2022)", "256GB", "excellent", 4000, "manual"),
            ("iPad", "iPad Pro 12.9 (M2, 2022)", "256GB", "good", 3200, "manual"),
            ("iPad", "iPad Pro 12.9 (M2, 2022)", "256GB", "fair", 2200, "manual"),
            ("iPad", "iPad Pro 11 (M2, 2022)", "128GB", "excellent", 2800, "manual"),
            ("iPad", "iPad Pro 11 (M2, 2022)", "128GB", "good", 2200, "manual"),
            ("iPad", "iPad Pro 11 (M2, 2022)", "128GB", "fair", 1500, "manual"),
            ("iPad", "iPad Pro 11 (M2, 2022)", "256GB", "excellent", 3200, "manual"),
            ("iPad", "iPad Pro 11 (M2, 2022)", "256GB", "good", 2600, "manual"),
            ("iPad", "iPad Pro 11 (M2, 2022)", "256GB", "fair", 1800, "manual"),
            # iPad Pro M1 (2021)
            ("iPad", "iPad Pro 12.9 (M1, 2021)", "128GB", "excellent", 2900, "manual"),
            ("iPad", "iPad Pro 12.9 (M1, 2021)", "128GB", "good", 2300, "manual"),
            ("iPad", "iPad Pro 12.9 (M1, 2021)", "128GB", "fair", 1500, "manual"),
            ("iPad", "iPad Pro 12.9 (M1, 2021)", "256GB", "excellent", 3300, "manual"),
            ("iPad", "iPad Pro 12.9 (M1, 2021)", "256GB", "good", 2600, "manual"),
            ("iPad", "iPad Pro 12.9 (M1, 2021)", "256GB", "fair", 1800, "manual"),
            ("iPad", "iPad Pro 11 (M1, 2021)", "128GB", "excellent", 2300, "manual"),
            ("iPad", "iPad Pro 11 (M1, 2021)", "128GB", "good", 1800, "manual"),
            ("iPad", "iPad Pro 11 (M1, 2021)", "128GB", "fair", 1200, "manual"),
            ("iPad", "iPad Pro 11 (M1, 2021)", "256GB", "excellent", 2700, "manual"),
            ("iPad", "iPad Pro 11 (M1, 2021)", "256GB", "good", 2100, "manual"),
            ("iPad", "iPad Pro 11 (M1, 2021)", "256GB", "fair", 1400, "manual"),
            # iPad Air M2 (2024)
            ("iPad", "iPad Air 11 (M2, 2024)", "128GB", "excellent", 2500, "manual"),
            ("iPad", "iPad Air 11 (M2, 2024)", "128GB", "good", 2000, "manual"),
            ("iPad", "iPad Air 11 (M2, 2024)", "128GB", "fair", 1300, "manual"),
            ("iPad", "iPad Air 11 (M2, 2024)", "256GB", "excellent", 2900, "manual"),
            ("iPad", "iPad Air 11 (M2, 2024)", "256GB", "good", 2300, "manual"),
            ("iPad", "iPad Air 11 (M2, 2024)", "256GB", "fair", 1500, "manual"),
            ("iPad", "iPad Air 13 (M2, 2024)", "128GB", "excellent", 3200, "manual"),
            ("iPad", "iPad Air 13 (M2, 2024)", "128GB", "good", 2600, "manual"),
            ("iPad", "iPad Air 13 (M2, 2024)", "128GB", "fair", 1700, "manual"),
            ("iPad", "iPad Air 13 (M2, 2024)", "256GB", "excellent", 3700, "manual"),
            ("iPad", "iPad Air 13 (M2, 2024)", "256GB", "good", 3000, "manual"),
            ("iPad", "iPad Air 13 (M2, 2024)", "256GB", "fair", 2000, "manual"),
            # iPad Air M1 (2022)
            ("iPad", "iPad Air (M1, 2022)", "64GB", "excellent", 1700, "manual"),
            ("iPad", "iPad Air (M1, 2022)", "64GB", "good", 1300, "manual"),
            ("iPad", "iPad Air (M1, 2022)", "64GB", "fair", 800, "manual"),
            ("iPad", "iPad Air (M1, 2022)", "256GB", "excellent", 2200, "manual"),
            ("iPad", "iPad Air (M1, 2022)", "256GB", "good", 1700, "manual"),
            ("iPad", "iPad Air (M1, 2022)", "256GB", "fair", 1100, "manual"),
            # iPad Air (第4代, 2020)
            ("iPad", "iPad Air (第4代, 2020)", "64GB", "excellent", 1200, "manual"),
            ("iPad", "iPad Air (第4代, 2020)", "64GB", "good", 900, "manual"),
            ("iPad", "iPad Air (第4代, 2020)", "64GB", "fair", 500, "manual"),
            ("iPad", "iPad Air (第4代, 2020)", "256GB", "excellent", 1600, "manual"),
            ("iPad", "iPad Air (第4代, 2020)", "256GB", "good", 1200, "manual"),
            ("iPad", "iPad Air (第4代, 2020)", "256GB", "fair", 700, "manual"),
            # iPad mini 6 (2021)
            ("iPad", "iPad mini (第6代, 2021)", "64GB", "excellent", 2100, "manual"),
            ("iPad", "iPad mini (第6代, 2021)", "64GB", "good", 1600, "manual"),
            ("iPad", "iPad mini (第6代, 2021)", "64GB", "fair", 1000, "manual"),
            ("iPad", "iPad mini (第6代, 2021)", "256GB", "excellent", 2600, "manual"),
            ("iPad", "iPad mini (第6代, 2021)", "256GB", "good", 2000, "manual"),
            ("iPad", "iPad mini (第6代, 2021)", "256GB", "fair", 1300, "manual"),
            # iPad mini 5 (2019)
            ("iPad", "iPad mini (第5代, 2019)", "64GB", "excellent", 800, "manual"),
            ("iPad", "iPad mini (第5代, 2019)", "64GB", "good", 600, "manual"),
            ("iPad", "iPad mini (第5代, 2019)", "64GB", "fair", 350, "manual"),
            # iPad 入門款 (第10代, 2022)
            ("iPad", "iPad (第10代, 2022)", "64GB", "excellent", 1400, "manual"),
            ("iPad", "iPad (第10代, 2022)", "64GB", "good", 1100, "manual"),
            ("iPad", "iPad (第10代, 2022)", "64GB", "fair", 700, "manual"),
            ("iPad", "iPad (第10代, 2022)", "256GB", "excellent", 1900, "manual"),
            ("iPad", "iPad (第10代, 2022)", "256GB", "good", 1500, "manual"),
            ("iPad", "iPad (第10代, 2022)", "256GB", "fair", 1000, "manual"),
            # iPad 入門款 (第9代, 2021)
            ("iPad", "iPad (第9代, 2021)", "64GB", "excellent", 1000, "manual"),
            ("iPad", "iPad (第9代, 2021)", "64GB", "good", 750, "manual"),
            ("iPad", "iPad (第9代, 2021)", "64GB", "fair", 450, "manual"),
            ("iPad", "iPad (第9代, 2021)", "256GB", "excellent", 1400, "manual"),
            ("iPad", "iPad (第9代, 2021)", "256GB", "good", 1100, "manual"),
            ("iPad", "iPad (第9代, 2021)", "256GB", "fair", 700, "manual"),
            # iPad 入門款 (第8代, 2020)
            ("iPad", "iPad (第8代, 2020)", "32GB", "excellent", 600, "manual"),
            ("iPad", "iPad (第8代, 2020)", "32GB", "good", 450, "manual"),
            ("iPad", "iPad (第8代, 2020)", "32GB", "fair", 250, "manual"),
            ("iPad", "iPad (第8代, 2020)", "128GB", "excellent", 900, "manual"),
            ("iPad", "iPad (第8代, 2020)", "128GB", "good", 700, "manual"),
            ("iPad", "iPad (第8代, 2020)", "128GB", "fair", 400, "manual"),
        ]
